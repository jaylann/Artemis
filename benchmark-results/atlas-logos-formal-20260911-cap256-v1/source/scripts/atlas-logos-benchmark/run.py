...
def manifest(
    ...
    return {
        "schemaVersion": 1,
        "campaignId": campaign,
        "campaignVersion": "atlas-logos-benchmark-v4-256",
...
def api(base: str, method: str, path: str, payload: Any = None, multipart: bool = False) -> Any:
...
def prepare(mode: str, offline: bool) -> int:
    campaign = os.environ.get("ATLAS_BENCHMARK_CAMPAIGN_ID", f"atlas-logos-{mode}-20260910")
    seed, repetitions = (
...
def enable_atlas_features(base: str) -> None:
    ...
    enabled_features = api(base, "PUT", "/api/admin/feature-toggle", {"AtlasAgent": True, "AtlasML": False})
...
def run(mode: str) -> int:
    ...
    for position, item in enumerate(prepared["schedule"], 1):
        ...
        try:
            ...
            condition, indexes = item["condition"], item["changedExerciseIndexes"]
