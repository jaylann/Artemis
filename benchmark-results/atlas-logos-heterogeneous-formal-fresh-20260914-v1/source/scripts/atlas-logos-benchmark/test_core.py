    ...
    def test_run_enables_atlas_before_first_course_update_after_restart(self):
        with (
            ...
            with (
                ...
                patch.object(run, "verify_models"),
                patch.object(run, "EvidenceGuard"),
...
class CostTests(unittest.TestCase):
    ...
    def test_long_context_prices_use_actual_tokens_and_include_cache_and_output(self):
        rate = self.pricing["models"]["model"]
        rate.update({"cacheWriteInputEurPerMillion": "1.25", "maxSupportedInputTokens": 1050000,
                     "longContext": {"thresholdTokens": 272000, "inputMultiplier": 2, "outputMultiplier": 1.5}})
        for inputs, expected in [(1000, "0.001095"), (272000, "0.272095"), (272001, "0.544092")]:
            with self.subTest(inputs=inputs):
                event = self.event | {"inputTokens": inputs, "cacheWriteInputTokens": 300}
                exports = {"req": self.exports["req"] | {"usage": self.exports["req"]["usage"] |
                           {"prompt_tokens": inputs, "prompt_cache_write_tokens": 300}}}
                self.assertEqual(report.attempt_cost(event, self.pricing, exports), (Decimal(expected), "matched"))

    def test_expanded_input_limit_requires_verified_long_context_rates(self):
        self.pricing["models"]["model"]["maxSupportedInputTokens"] = 1050000
        self.assertEqual(report.attempt_cost(self.event, self.pricing, self.exports), (None, "price-unverified"))
